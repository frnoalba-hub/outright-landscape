  ...
  return (
    <div className="min-h-screen bg-white" role="document">
      ...
      scrolled ? 'bg-[#1a1a1a]/95 backdrop-blur-md shadow-lg py-1 sm:py-1.5' : 'bg-[#1a1a1a]/70 backdrop-blur-sm py-2 sm:py-2.5'}`
        ...
        <nav className="max-w-7xl mx-auto px-4 sm:px-6 xl:px-8" aria-label="Main navigation">
          <div className="flex items-center justify-between">
            <a
              href={createPageUrl("Home")}
              className="headerLogo flex flex-shrink-0 items-center gap-2 xl:gap-3 group"
              ...
              <span className="headerBrandName max-w-[8.5rem] xl:max-w-none font-bold text-base sm:text-lg xl:text-xl leading-tight text-white">Outright Landscape</span>
              <span className="headerLicense text-[10px] sm:text-[11px] text-[#b8945a]/70 font-medium tracking-wider">CSLB #1073845</span>
              </div>
            </a>
            
            <ul className="headerNav hidden xl:flex items-center space-x-5 2xl:space-x-8">
              {navigationItems.map((item) => <li key={item.title}>
                  <a
                    href={item.href}
                    className="headerNavLink text-white/70 hover:text-[#c45d2c] font-medium transition-colors text-sm 2xl:text-base tracking-wide py-2 whitespace-nowrap">
                    {item.title}
                  </a>
                </li>
              )}
