...
export default function PasadenaLandscaping() {
  const [formData, setFormData] = useState({ name: "", phone: "", email: "", city: "Pasadena", message: "", service_type: "landscape_design" });
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isSubmitted, setIsSubmitted] = useState(false);

  const handleSubmit = async (e) => { e.preventDefault(); setIsSubmitting(true); try { await base44.entities.ContactInquiry.create(formData); setIsSubmitted(true); } catch (err) { console.error("Submission failed", err); alert("Sorry, couldn't send your request. Please call (626) 343-6028."); } setIsSubmitting(false); };
