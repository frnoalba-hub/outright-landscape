// Base44 Safe Haptic Feedback Integration (with built-in iOS support)
// ------------------------------------------------------------------
// This module replicates the ios-haptics library internally and combines it
// with the navigator.vibrate API. No external scripts or dynamic imports.

import { useCallback, createContext, useContext } from 'react';

// Detect if we're on an iOS device (iPhone/iPad) using Safari 17.4+
const isIOS = () => {
  if (typeof navigator === 'undefined') return false;
  // Basic iOS device detection
  const isApple = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
  // Safari 17.4 introduced haptics on switches; older versions will ignore haptic()
...
const useHapticFeedback = () => {
  return useCallback((type = 'light') => {
    if (isIOS()) {
      // Map our generic types to ios-haptics patterns
      switch (type) {
        case 'success':
        case 'notification':
        case 'medium':
...
// React context for sharing triggerHaptic
